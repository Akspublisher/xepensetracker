...
export default function ExpenseForm({ setIsOpen, expensesList, setExpensesList, editId, setBalance, balance }) {
    ...
    const handleAdd = (e) => {
        ...
        const lastId = expensesList.length > 0 ? expensesList[0].id : 0
        setExpensesList(prev => [{ ...formData, id: lastId + 1 }, ...prev])
    ...
    useEffect(() => {
        ...
        if (editId) {
            const expenseData = expensesList.find(item => item.id === editId)
        ...
        <div className={styles.formWrapper}>
            ...
            <form onSubmit={editId ? handleEdit : handleAdd}>
                ...
                <Button
                 ...
                 type="submit" style="primary" shadow>{editId ? 'Edit Expense' : 'Add Expense'}</Button>

                <Button
                 // eslint-disable-next-line react/style-prop-object
                 style='secondary' shadow
